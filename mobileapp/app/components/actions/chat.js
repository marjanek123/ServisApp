...
export const getChatMessages = (sender, receiver) => (dispatch, getState) => {
    ...
    axios
        .get(
            `http://${HOST}/api/messages/${sender.id}/${receiver}/50/`,
...
export const getChatGroupMessages =
    (group, messagesInt) => (dispatch, getState) => {
        ...
        axios
            .get(
                `http://${HOST}/api/messages-group/${group}/${messagesInt}/`,
                tokenConfig(getState)
            )
            .then((res) => {
                dispatch({
...
export const SendChatGroupMessages =
    (group, messagesInt, body) => (dispatch, getState) => {
        ...
        axios
            .post(
                `http://${HOST}/api/messages-group/${group.id}/${messagesInt}/`,
                body,
                tokenConfig(getState)
            )
            .then((res) => {
...
export const reloadChatMessages =
    (sender, receiver, messagesInt) => (dispatch, getState) => {
        ...
        axios
            .get(
                `http://${HOST}/api/messages/${sender}/${receiver}/${messagesInt}/`,
                tokenConfig(getState)
            )
            .then((res) => {
                dispatch({
...
export const sendMessage =
    (sender, receiver, formdata) => (dispatch, getState) => {
        ...
        axios
            .post(
                `http://${HOST}/api/messages/${sender.id}/${receiver.id}/1/`,
                formdata,
                tokenConfig(getState)
            )
            .then((res) => {
